...
export function SidebarMenu({ user }: { user: User }) {
  ...
  return (
    <>
      ...
      {/* Sidebar / Drawer */}
      <div 
        className={`fixed top-0 right-0 h-full w-64 sm:w-80 bg-[#09090b] border-l border-zinc-800 z-[101] shadow-[-10px_0_30px_rgba(225,0,0,0.1)] transform transition-transform duration-300 ease-in-out ${
      ...
      >
        <div className="flex flex-col h-full p-6">
          <div className="flex justify-between items-center mb-8 pb-4 border-b border-zinc-800">
            ...
            <button 
              onClick={() => setIsOpen(false)}
              className="text-zinc-500 hover:text-white text-2xl font-light"
            >
              &times;
            </button>
          </div>
